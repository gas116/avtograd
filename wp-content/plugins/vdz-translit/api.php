...
add_action(VDZ_TRANSLIT_API, function($action = '', $plugin_name = ''){
    ...
    if ( ! is_wp_error( $response ) && isset($response['body']) && (json_decode($response['body']) !== null ) ){
        ...
        set_transient( VDZ_TRANSLIT_API . '_activation_notice_text', $response['body'], 60 );
    }
}, 10, 2);
...
add_action( 'admin_notices', function () {
    ...
    $message = get_transient( VDZ_TRANSLIT_API . '_activation_notice_text' );
    ...
    if( !empty($all_plugins) ){
        ...
        printf( '<div id="'.VDZ_TRANSLIT_API.'" class="%1$s">%2$s</div>', $class, $msg );
        //Мгновенное удаление временного сообщения
        delete_transient( VDZ_TRANSLIT_API . '_activation_notice_text' );
    }
});
